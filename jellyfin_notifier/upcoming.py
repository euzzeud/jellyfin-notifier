...
# Guards each function's whole load-modify-save below, not just the final
# write: this is admin-only (no background thread writes here), but two
# add/delete/save_poster calls landing at once (e.g. a double-click, or two
# browser tabs) would otherwise still be a lost-update race even with an
# atomic write at the end - the second save would silently overwrite
# whatever the first one just added.
